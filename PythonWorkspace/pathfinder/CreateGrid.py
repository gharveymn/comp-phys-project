...
def createGraphDict(x,y,z):
	...
	#This is bijective
	indsPtsMap = dict(zip([i for j in allInds for i in j], [i for j in allPoints for i in j]))
	ptsIndsMap = dict(zip([i for j in allPoints for i in j], [i for j in allInds for i in j]))
