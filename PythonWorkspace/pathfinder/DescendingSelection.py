...
def tester():
	...
	bounds = (0.,5.,0.,10.)
	randArr = list(zip(list((bounds[1]-bounds[0])*random.random(20)),list((bounds[3]-bounds[2])*random.random(20))))
	order = 1
...
def makeRandomArray(numPoints, bounds):
	return list(zip(list((bounds[1]-bounds[0])*random.random(numPoints)),list((bounds[3]-bounds[2])*random.random(numPoints))))
pass


def makeColorDict(pointArray):
	return {p:(0.1+0.9*random.random(),0.1+0.9*random.random(),0.1+0.9*random.random()) for p in pointArray}
pass
...
def getScatter(mdict, persistentorder, order, clr, bounds, absPosX = 0, absPosY = 0, ret = []):
	...
	if order == -1:
		...
		for p in mdict:
			ret = ret + getScatter(mdict[p], persistentorder, order-1, clr, bounds, absPosX+(p[0])*mltplrx, absPosY+(p[1])*mltplry)
		pass
