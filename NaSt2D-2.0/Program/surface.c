...
/* Fluessigkeitsbereich in Abhaengigkeit von problem.                     */
/*------------------------------------------------------------------------*/
                                     ...
                                     int ppc,char *problem,REAL **U,REAL **V)
{
 ...
 REAL height=0,rad=0,mpx=0,mpy=0,vstart=0,
               rad1=0,mpx1=0,mpy1=0,vstart1=0,
               rad2=0,mpx2=0,mpy2=0,vstart2=0;        /* Groessen fuer Tropfen */
 ...
 if(strcmp(problem, "tropfen1")==0) /* Ein Tropfen */
   {
    *N = 2;
    height = 1./2.*jmax*dely;       /* Wasserbecken */
    rad    = 0.1*jmax*dely;         /* Radius des Tropfens */
    mpx    = 0.5*imax*delx;         /* Mittelpunkt des Tropfens */
    mpy    = 2./3.*jmax*dely;
    vstart = -2.0;                  /* Anfangsgeschwindigkeit Tropfen */
   }

 if(strcmp(problem, "tropfen2")==0) /* Zwei Tropfen */
   {
    *N = 3;
    height  = 1./2.*jmax*dely;      /* Wasserbecken */
    rad1    = 0.1*jmax*dely;        /* Radius des Tropfen1 */
    mpx1    = 0.3*imax*delx;        /* Mittelpunkt des Tropfen1 */
    mpy1    = 2./3.*jmax*dely;
    vstart1 = -2.0;                 /* Anfangsgeschwindigkeit Tropfen1 */
    rad2    = 0.12*jmax*dely;       /* Radius des Tropfen2 */
    mpx2    = 0.7*imax*delx;        /* Mittelpunkt des Tropfen2 */
    mpy2    = 2./3.*jmax*dely;
    vstart2 = -2.2;                 /* Anfangsgeschwindigkeit Tropfen2 */
   }
          ...
          for (jp=1;jp<=ppc;jp++)
	    {
                ...
                if (y<height)
		  {
                   SET_PART(&Particlelines[1],x,y);
                      ...
                      if ((x-mpx1)*(x-mpx1)+(y-mpy1)*(y-mpy1) <= rad1*rad1)
		  {
                   SET_PART(&Particlelines[2],x,y);
                   V[i][j] = vstart1;
		  };
                      if ((x-mpx2)*(x-mpx2)+(y-mpy2)*(y-mpy2) <= rad2*rad2)
		  {
                   SET_PART(&Particlelines[3],x,y);
                   V[i][j] = vstart2;
		  };
	        }
              }
...
void SET_PART(struct particleline *Partline, REAL x,REAL y)
{
 ...
 part->next = (*Partline).Particles->next;   /* Kette an part anhaengen */
 ...
 for (j=1;j<=jmax;j++) 
    for (i=1;i<=imax;i++)
	  ...
	  default     : break;
                        ...
                        /* Zweiter Durchlauf fuer Druckrandwerte */
                        /*---------------------------------------*/
 for (j=1;j<=jmax;j++)
    for (i=1;i<=imax;i++)
      {
